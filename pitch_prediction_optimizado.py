# 📦 Requisitos
# pip install streamlit pybaseball pandas scikit-learn xgboost
...
def train_model(df, casino_line):
    ...
    st.info(f"Se encontraron {len(df)} registros para entrenar el modelo.")
...
# --- Streamlit UI ---
st.title("Predicción Optimizada de Velocidad del Pitcheo: OVER / UNDER")

tab1, tab2 = st.tabs(["Primer Pitcheo", "En Juego (In-Play)"])

with tab1:
    st.header("Predicción para Primer Pitcheo")
    pitcher = st.text_input("Nombre del Pitcher", "Gerrit Cole", key="p1")
    batter = st.text_input("Nombre del Bateador", "Mike Trout", key="b1")
...
with tab2:
    ...
    pitcher = st.text_input("Nombre del Pitcher", "Gerrit Cole", key="p2")
    batter = st.text_input("Nombre del Bateador", "Mike Trout", key="b2")
